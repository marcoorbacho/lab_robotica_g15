...
class TurtlebotController():
    ...
    def command(self):
        ...
        if(self.goal_received == True):
            ...
            try:
                ...
                self.tf_listener.transformPoint('base_link', goal, base_goal)
    ...
    def Obstaculo_delante_dir_wp(self,angle):
        ...
        num_readings = len(self.scan_data.ranges) # Número total de lecturas del LIDAR
        angle_increment = self.scan_data.angle_increment # Incremento angular por lectura
        center_index = num_readings // 2        # Índice central del LIDAR

        #Calcular la distancia que hay en la dirección del ángulo dado
        angle_rad = angle * (math.pi / 180)  # Convertir a radianes
        target_index = center_index + int(angle_rad / angle_increment)  # Índice objetivo
        distance_at_angle = self.scan_data.ranges[target_index]
        obstaculo.dist_objetivo = distance_at_angle
    
        # Ángulo de 15 grados a cada lado
        angle_range = 15  
        rango_angulo = int(angle_range * (math.pi / 180) / angle_increment)    # Índices que corresponden a 15 grados

        # Buscar la distancia mínima en la ventana definida
        min_distance = min(self.scan_data.ranges[center_index - rango_angulo : center_index + rango_angulo + 1])
        # Devolver el ángulo de la distancia mínima al obstáculo y el valor de la distancia
        min_index = self.scan_data.ranges.index(min_distance, center_index - rango_angulo, center_index + rango_angulo + 1)
        obstaculo.angle = (min_index - center_index) * angle_increment
        obstaculo.distance = min_distance
        return obstaculo        
